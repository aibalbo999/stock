...
外部部署接點可集中用 `.venv/bin/python scripts/external_integrations_smoke.py --json` 檢查；一般模式只回報 `caution`，若正式部署要求 Neo4j live query/import、Browser/Proxy fallback 與結構化文件 API 都就緒，可加 `--strict` 讓任一項未 ready 時回傳非 0 exit code。JSON 會在每個 check 內列出 `smoke_commands`，純文字輸出也會在各項下方顯示可複製的 smoke 指令；其中 `company_filing_structured_api_sample_contract` 會離線驗證 `examples/structured_company_filing_sample.json` 是否仍能轉成系統文件，用來區分「本地 mapping contract 正常」與「live TEJ/資料商尚未配置」。若只想驗證 Neo4j GraphRAG contract，可單獨執行 `.venv/bin/python scripts/neo4j_graphrag_smoke.py --tickers 2330 --target-ticker 2382 --question 上下游衝擊 --json`；live Cypher 的 upgrade audit evidence 會同時帶出 Neo4j payload dry-run、read-only query smoke 與 `--import-first` smoke，方便區分 payload、連線、匯入與查詢問題。若只想驗證結構化公司文件 API contract，可單獨執行 `.venv/bin/python scripts/structured_company_filing_smoke.py --ticker 2330 --company-name 台積電 --document-type investor_presentation --json`。
...
若第一次啟動時本機尚未有 Redis / Postgres / Neo4j / Browserless / Chroma image，一鍵啟動會先停在 `需下載` 並提示 `docker compose pull redis postgres neo4j browserless chroma`，避免安靜卡在 Docker pull。依賴啟動回報 `需下載` 或 `失敗` 時，流程會在資料庫 migration 前停止，讓錯誤直接指向 Docker image、pull 或 Compose 問題，而不是後續 Alembic/API 的連線錯誤。確認網路與 Docker Desktop 正常後，可先手動 pull，或改用 `.venv/bin/python scripts/start_system.py --start-dependencies --pull-missing-dependencies` 允許啟動流程自動下載；自動下載會先逐一 pull 缺少的 service image，再重新檢查 image 是否存在，若 Docker Desktop 網路或 registry 逾時，訊息會指出卡在哪一個 service。啟動依賴時，本次程序也會套用 `USE_CHROMA=true` 與 `CHROMA_API_URL=http://127.0.0.1:8001`，讓本機 RAG 可使用 Chroma HTTP server，且不改寫 `.env`。遇到 MOPS / IR 入口被 Cloudflare、CAPTCHA 或空殼頁擋住時，可加 `--prefer-unlocker`，一鍵啟動會用 docker-compose `unlocker` profile 啟動 FlareSolverr，並在本次程序把公司文件 render provider 設為 `flaresolverr`；若 FlareSolverr 8191 尚未就緒但 Browserless 3000 已就緒，會自動退回 Browserless，兩者都不可用且本機 Playwright 套件與 Chromium binary 已安裝時，才切到 Playwright。這些臨時 env 不會改寫 `.env`。
...
服務啟動後可跑 `.venv/bin/python scripts/frontend_smoke.py --json`，它會檢查 Streamlit 首頁、FastAPI `/services/status`、`pages/` 多頁入口與 `app.ui.streamlit_dashboard` facade 的匯入合約，並在 Playwright 可用時截圖到 `artifacts/frontend_smoke/streamlit.png` 且驗證畫面不是空白 PNG。CI 或沒有瀏覽器 binary 的環境可加 `--skip-browser`，仍保留 HTTP 與多頁匯入合約 smoke。
...
報告寫入時會自動執行 latest-per-topic retention：同一分析主題產生新版後，舊版報告會被刪除，舊 analysis run 的 report link 會清空，對應的舊 markdown 檔也會被清掉，避免報告中心與 `reports/` 累積同主題歷史版本。手動刪除報告也會解除 analysis run 的 report link/output path，並且只移除設定的 `REPORT_DIR` / `report_dir` 底下對應的 `.md` 檔，避免誤刪外部路徑。若資料庫已有早期累積的重複報告或 markdown 檔，可在系統設定頁「進階：資料清理」勾選確認後按「套用最新版報告保留策略」，或呼叫 `POST /maintenance/cleanup` 並送出 `{"latest_reports_only": true, "orphan_report_refs": true}`。`GET /services/status` 的 `upgrade_capability_matrix.data_business_logic.latest_report_retention` 會檢查 DB 寫入 prune、報告中心 latest-by-topic、品質摘要、maintenance cleanup、手動刪除清理與 markdown 檔清理路徑是否仍完整，並在暫存目錄執行 markdown retention smoke，確認 timestamp 與 legacy 檔名都只保留每主題最新版，避免後續改版又累積同主題舊報告。

系統設定頁的「AI 用量趨勢與成本」會讀取 `GET /llm/usage/summary?days=7`，顯示 7 日 request/token、成本估算、fallback path、retryable failure、成本預算狀態與 alerts，並依模型、任務與日期彙總，方便確認免費額度是否被有效使用。「AI 額度與模型路由」會讀取 `GET /llm/quota`，顯示推薦模型、推薦原因、每模型 routing tier/status reason 與高額度保底模型，並從 `GET /services/status` 顯示 Visual RAG / PDF 圖片解析專用模型鏈，列出 Vision 候選、request budget、hard routing 與 Gemma/Imagen/Embedding/Live 排除原因。維護頁的「外部部署選配狀態」會把 upgrade audit 的 `failures`、`warnings` 與 `optional_warnings` 中的外部整合項目列成表格，並集中顯示 Neo4j payload/import、GraphRAG live Cypher、公司文件 render fallback、高風險 MOPS/TWSE/TPEx unlocker 與結構化文件 API 的單項 smoke 指令；Neo4j/GraphRAG 會另列本機 start_system、upgrade audit、payload dry-run、live/import-first smoke 與 Docker logs 操作提示，高風險 unlocker 會另列 provider/tier、fallback reason、建議 env 與 MOPS smoke，結構化文件 API 會另列 provider env、sample contract、live smoke、request contract 與 required fields。正式部署可使用 `scripts/external_integrations_smoke.py --strict --json`。「報告生成觀測」會讀取 `GET /reports/observability/summary?limit=20`，彙總 latest-per-topic 報告的 LLM latency、retrieval latency、token/cost、fallback、reranker 與 GraphRAG trace，並列出 report-level bottleneck、dominant factor 與 next action，優先處理最耗額度或最常降級的報告。「背景任務觀測」會先顯示 `task_queue` 的 Queue / Worker readiness 診斷表格與 Celery smoke 指令，再讀取 `GET /tasks/summary?days=7` 顯示近期 Celery/API run 成功率、平均耗時、失敗任務與疑似卡住任務。`task_summary` 會解析 analysis run payload 中的 `celery_task_id`、operation 與 retry kind，為每個失敗列輸出 `retryable`、`retry_endpoint`、`status_endpoint`、`error_category`、`error_severity`、`next_steps` 與 `next_action`；`GET /tasks/{task_id}` 也會在單一任務失敗時輸出同樣診斷欄位，任務狀態面板會顯示「失敗診斷」表格，且公司文件補抓成功結果會先把 `next_actions` / `gap_summary` 轉成公司文件補抓摘要表，再保留完整 JSON drilldown。目前會分類 quota、task_queue、payload_validation、timeout、visual_rag、data_source、cancelled 與 unknown，並提供 `by_error_category` 總量聚合、`error_category_daily` 每日趨勢與 `alerts` 優先處理提醒。`AnalysisRunRepository.mark_failed/mark_cancelled` 會把同一份診斷寫回 `payload.task_failure_diagnostic`，後續 summary/status 會優先使用持久化診斷，避免 Celery backend 過期後失去歷史失敗原因。維護頁會把近期失敗轉成 drilldown 表格，支援的失敗任務可直接呼叫 `POST /tasks/{task_id}/retry`，並用任務狀態面板追蹤新 task id。「報告品質 Gate 總覽」會讀取 `GET /reports/quality/summary?limit=20`，以 latest-per-topic 報告檢查 blockers、warnings、正式分析信心與資料覆蓋。
...
核心檢查失敗時 CLI 會回傳非 0 exit code；免費額度感知模型路由屬於核心檢查，若模型順序、同級 Flash budget、Gemma 高額度保底或 hard routing/cooldown 設定退化，會被視為 implementation failure。只有外部選配能力（例如 live Neo4j import、Visual RAG runtime、PDF 表格 runtime、公司文件 Proxy / Browser render / Playwright 後援、高風險 MOPS/TWSE/TPEx unlocker、結構化文件 API）未設定時，預設回傳 0 但標示 `caution`。CLI、API 與維護頁會同時顯示 `implementation`（核心升級）與 `deployment`（外部整合）狀態，因此可分辨是分析/RAG/資料邏輯尚未就緒，還是 Neo4j、瀏覽器渲染、CAPTCHA unlocker 或代理這類正式部署服務尚未啟動。搭配 `--strict-external` 可把外部整合也列為必須通過。
