...
萃取結果會加入 `[Visual RAG 解析資訊]` 標記，記錄 mode、renderer、頁數、model、觸發原因與 observability 摘要；`GET /services/status` 與 upgrade audit 會回傳 `visual_rag` runtime，讓部署前能分辨是尚未啟用、缺 PyMuPDF，還是缺 vision LLM key。
