...
一鍵啟動會使用同一套稽核輸出 preflight 摘要；若要把外部整合也列為必須通過，可執行 `.venv/bin/python scripts/start_system.py --start-dependencies --strict-upgrade-check`。搭配 `--start-dependencies` 時，啟動程式會先等本機 Redis 6379、Postgres 5432、Neo4j 7687、Browserless 3000、Chroma 8001，以及 `--prefer-unlocker` 時的 FlareSolverr 8191 連線埠短暫就緒再跑稽核，降低 Docker 剛啟動時的誤判。
