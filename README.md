...
- 系統狀態檢查：`service_status` 負責彙整整體狀態，前端 MPA/背景任務 ready 條件、LLM 額度/降級路由、vector store/reranker runtime、市場資料快取/來源後援、GraphRAG/Neo4j runtime、Python runtime、最新版報告保留、security scanning 與公司文件爬蟲/PDF runtime 由獨立 status collector 掃描；外部部署的 readiness 計算與 profile catalog 也拆成獨立 service，`upgrade_capability_matrix` 由獨立 builder 組裝；`optimization_progress` 會把原始優化方案彙整成「架構/UI、維護性、資料管線、AI/RAG」四大主題的完成率、blocking gap、外部選配、下一步、`prioritized_next_actions` 優先隊列與 `local_resolution_projection` 本機 defaults 投影，避免單一 audit 檔案持續膨脹，也避免把付費/外部部署選配誤判成一般開發阻斷。
...
LLM_MODEL_DAILY_REQUEST_BUDGETS=gemini-3.5-flash=250,gemini-2.5-flash=250,gemini-3.1-flash-lite=250,gemini-2.5-flash-lite=1000,gemma-4-31b-it=14400
...
免費版 API key 目前採取智慧優先策略：`gemini-3.5-flash` 作為正式報告、GraphRAG 推理、結構化補充分析、LLM reranker 與 Visual RAG PDF 圖片解析的主模型；只有該模型回傳 429/quota、上游錯誤或空回覆時，才依序降級到 `gemini-2.5-flash`、`gemini-3.1-flash-lite`、`gemini-2.5-flash-lite`，最後才使用高額度保底的 `gemma-4-31b-it`。`gemini-embedding-2` 作為 RAG embedding。Google 的 Gemini API rate limits 以 project 計算，不是以 API key 計算，因此 `GOOGLE_API_KEYS` 多把 key 只用於輪替錯誤/分散瞬時失敗，不代表免費 RPD/TPM 會倍增。`gemini-3.5-flash` 保留為第一順位；`gemini-3.5-flash` 與 `gemini-3.1-flash-lite` 是依你的 AI Studio/project 可用性保留的自訂模型，預設以 250 RPD 追蹤並可用 `LLM_MODEL_DAILY_REQUEST_BUDGETS` 覆寫；Google 官方 Free Tier 表列 `gemini-2.5-flash` 為 250 RPD、`gemini-2.5-flash-lite` 為 1,000 RPD、`Gemini Embedding` 為 1,000 RPD、`Gemma 3 & 3n` 為 14,400 RPD，本專案同步用這些值當 baseline，`gemma-4-31b-it` 則保留為高量文字任務保底並以 AI Studio 顯示額度為準。若本機 smoke test 回傳 quota 429，通常代表該 project 今日或當前視窗額度已耗盡。系統會在模型回傳 429 時套用 `LLM_MODEL_QUOTA_COOLDOWN_SECONDS`，短時間跳過該模型，避免每次報告都先撞已耗盡的模型；cooldown 結束後會自動再次把高階模型放回優先嘗試。這個 cooldown 現在同時看 runtime memory 與 persisted attempts：`GET /llm/quota` 會把近期 429 計為 `quota_hit_count`、把 hard-routing/cooldown skip 計為 `quota_skip_count`，並用 `active_cooldown_seconds` 顯示重啟後仍可參考的冷卻時間。`LLM_QUOTA_WARNING_RATIO` 只用來在用量接近設定額度時顯示 amber alert，不會提前降級；路由仍等模型實際 exhausted 或 active cooldown 才跳到下一順位。`GET /llm/quota` 與系統設定頁會顯示 Pacific-day 額度視窗、今日已用 request/token、每模型剩餘估算、推薦原因、risk level、near-limit/cooldown alert、每個模型的 routing tier/status reason、quota health、高額度保底模型與 `free_tier_reference` 官方參考來源；request 用量會優先依 persisted attempts 歸因到實際嘗試過的模型，沒有 attempts 時才退回 `models_tried` 或 final model，讓 429 後降級的前置模型也能納入 hard routing 判斷。`GET /services/status` 的 `upgrade_capability_matrix.ai_rag.llm_quota_routing` 會檢查 `gemini-3.5-flash` 第一順位、智慧優先模型 request budget、官方 Free Tier 參考值、Gemma 高額度保底、hard routing/cooldown、quota warning threshold、embedding 模型分離與 Imagen/Live 不進報告路由。實際限制仍以 Google AI Studio 顯示的 project limit 為準。Imagen / Live 模型偏圖片生成或即時語音互動，不進入目前報告與資料管線核心流程。
...
維護頁的結構化文件 API 操作提示會分開列出 `Sample contract`、`Local fixture HTTP` 與 `Live smoke`：免費版可先完成前兩項，確認 JSON mapping 與 HTTP fetch path 正常；sample/live/fixture smoke 會輸出 `contract_diagnostics`，包含 row container、conversion ratio、title/text/ticker/company/document_type 欄位覆蓋，方便接 TEJ 或 custom API 前先定位 response contract 問題；`Local fixture HTTP` 會優先提供 `.venv/bin/python scripts/structured_company_filing_fixture_smoke.py --json --strict` 一鍵診斷，也保留 fixture server 與 live smoke 的拆步驟指令；只有要接 TEJ/資料商 live API 時才需要 provider URL/token。
