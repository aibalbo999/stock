...
萃取結果會加入 `[Visual RAG 解析資訊]` 標記，記錄 mode、renderer、頁數、model、觸發原因與 observability 摘要；若文字 parser 失敗且 Visual RAG 後援也失敗，錯誤訊息會保留缺 key、缺 PyMuPDF、模型不支援、額度耗盡或 LLM 失敗等實際原因，並在公司文件 gap summary 中輸出 `visual_rag_setup_tickers` 與 `visual_rag_review_tickers`。`GET /services/status` 與 upgrade audit 會回傳 `visual_rag` runtime，讓部署前能分辨是尚未啟用、缺 PyMuPDF，還是缺 vision LLM key。調整 VLM prompt 或模型順序後，可用 `.venv/bin/python scripts/evaluate_visual_rag.py --golden data/visual_rag_golden.jsonl --results visual_results.json --fail-under 1.0` 對已產出的抽取文字做 golden-set 比對；此評估不會呼叫 LLM，因此不消耗 Gemini 免費額度。
...
GitHub Actions workflow 位於 `.github/workflows/ci.yml`，會自動執行 `ruff check .`、`scripts/security_scan.py --engine detect-secrets`、`pytest -q`、`scripts/upgrade_audit.py --json`、外部整合 smoke、結構化公司文件 sample contract smoke、Visual RAG golden eval，以及啟動 API/Streamlit 後的 `scripts/frontend_smoke.py --skip-browser --json`。
...
核心檢查失敗時 CLI 會回傳非 0 exit code；免費額度感知模型路由屬於核心檢查，若模型順序、同級 Flash budget、Gemma 高額度保底或 hard routing/cooldown 設定退化，會被視為 implementation failure。只有外部選配能力（例如 live Neo4j import、Visual RAG runtime、PDF 表格 runtime、公司文件 Proxy / Browser render / Playwright 後援、結構化文件 API）未設定時，預設回傳 0 但標示 `caution`。CLI、API 與維護頁會同時顯示 `implementation`（核心升級）與 `deployment`（外部整合）狀態，因此可分辨是分析/RAG/資料邏輯尚未就緒，還是 Neo4j、瀏覽器渲染或代理這類正式部署服務尚未啟動。搭配 `--strict-external` 可把外部整合也列為必須通過。
