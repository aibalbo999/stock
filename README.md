...
萃取結果會加入 `[Visual RAG 解析資訊]` 標記，記錄 mode、renderer、頁數、model、觸發原因與 observability 摘要；若文字 parser 失敗且 Visual RAG 後援也失敗，錯誤訊息會保留缺 key、缺 PyMuPDF、模型不支援或 LLM 失敗等實際原因。`GET /services/status` 與 upgrade audit 會回傳 `visual_rag` runtime，讓部署前能分辨是尚未啟用、缺 PyMuPDF，還是缺 vision LLM key。調整 VLM prompt 或模型順序後，可用 `.venv/bin/python scripts/evaluate_visual_rag.py --golden data/visual_rag_golden.jsonl --results visual_results.json --fail-under 1.0` 對已產出的抽取文字做 golden-set 比對；此評估不會呼叫 LLM，因此不消耗 Gemini 免費額度。
