...
若只想驗證本機 GraphRAG reasoning contract，可執行 `.venv/bin/python scripts/evaluate_graphrag_reasoning.py --golden data/graphrag_reasoning_golden.jsonl --fail-under 1.0 --json`。此 golden eval 會檢查 taxonomy shortest path context、同業/上下游方向標籤、no-path evidence policy、deterministic Cypher plan 與 read-only guardrail，不會呼叫 LLM 或 Neo4j，因此不消耗 Gemini 免費額度。
