...
Chroma 對外 port 使用 `8001`，FlareSolverr unlocker 與 host/compose env 差異請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。

若公開資訊觀測站或公司 IR 入口遇到 Cloudflare / CAPTCHA / 空殼頁，可額外啟動 FlareSolverr unlocker profile；完整 provider 與 smoke 指令在 runbook：
...
GraphRAG/Neo4j 詳細端點、host/compose env、Homebrew 備援與 live import/query smoke 請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。最常用檢查：
...
公司文件抓取會支援 UA/proxy 輪換、403/429/5xx retry、PDF/HTML 表格抽取、Browserless/Playwright/FlareSolverr 後援、官方重大訊息 OpenAPI、structured filing API 與 Visual RAG。provider contract、MOPS unlocker、TEJ/custom API、錯誤分類與 smoke 指令請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。

PDF parser 與 Visual RAG 常用安裝：
...
若第一次啟動時本機尚未有 Redis / Postgres / Neo4j / Browserless / Chroma image，一鍵啟動會提示先 pull，並在 dependency/migration 前停止，避免後續錯誤誤指向 API 或 Alembic。自動下載、`--prefer-unlocker`、FlareSolverr fallback、本機 defaults 與維護頁後續診斷請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。
...
`upgrade_audit.py` 會讀取 `GET /services/status` 同源能力矩陣。預設模式保留外部部署提醒但不把 optional Neo4j、unlocker、Visual RAG、PDF parser 或 structured API 選配視為核心失敗；`--strict-external` 才會把外部整合納入必須通過項。`--auto-local-defaults`、等待本機 Neo4j/Chroma/Browserless/FlareSolverr、API 查詢與 preflight 細節請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。
...
系統設定頁會提供 AI 用量與成本、AI 額度與模型路由、外部部署選配、報告生成觀測、背景任務觀測與報告品質 Gate。外部部署面板會用 `external_deployment_local_projection` 顯示扣除本機 defaults 後的有效剩餘缺口；背景任務面板則提供 read-only 與 safe no-op submission smoke。維護頁欄位、env check、安全遮罩與診斷指令細節請見 [deployment readiness runbook](docs/runbooks/deployment-readiness.md)。
...
- `docs/runbooks/`：外部部署、公司文件抓取、GraphRAG/Neo4j、Visual RAG 與 smoke test 操作手冊
- `tests/`：單元與整合測試
