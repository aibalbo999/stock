...
維護頁的結構化文件 API 操作提示會分開列出 `Sample contract`、`Local fixture HTTP` 與 `Live smoke`：免費版可先完成前兩項，確認 JSON mapping 與 HTTP fetch path 正常；只有要接 TEJ/資料商 live API 時才需要 provider URL/token。
