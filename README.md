...
系統設定頁的「AI 用量趨勢與成本」會讀取 `GET /llm/usage/summary?days=7`，顯示 7 日 request/token、成本估算、fallback path、retryable failure、成本預算狀態與 alerts，並依模型、任務與日期彙總，方便確認免費額度是否被有效使用。「AI 額度與模型路由」會讀取 `GET /llm/quota`，顯示推薦模型、推薦原因、每模型 routing tier/status reason 與高額度保底模型。「背景任務觀測」會先顯示 `task_queue` 的 Queue / Worker readiness 診斷表格與 Celery smoke 指令，再讀取 `GET /tasks/summary?days=7` 顯示近期 Celery/API run 成功率、平均耗時、失敗任務與疑似卡住任務。`task_summary` 會解析 analysis run payload 中的 `celery_task_id`、operation 與 retry kind，為每個失敗列輸出 `retryable`、`retry_endpoint`、`status_endpoint`、`error_category`、`error_severity`、`next_steps` 與 `next_action`；`GET /tasks/{task_id}` 也會在單一任務失敗時輸出同樣診斷欄位，任務狀態面板會顯示「失敗診斷」表格。目前會分類 quota、task_queue、payload_validation、timeout、data_source、cancelled 與 unknown，並提供 `by_error_category` 聚合。維護頁會把近期失敗轉成 drilldown 表格，支援的失敗任務可直接呼叫 `POST /tasks/{task_id}/retry`，並用任務狀態面板追蹤新 task id。「報告品質 Gate 總覽」會讀取 `GET /reports/quality/summary?limit=20`，以 latest-per-topic 報告檢查 blockers、warnings、正式分析信心與資料覆蓋。
