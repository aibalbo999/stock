...
class VectorStore:
    ...
    def embedding_provider_status(
        ...
        settings = settings or get_settings()
        provider = cls._normalized_embedding_provider(settings)
        model_name = str(settings.rag_embedding_model or "").strip()
        default_status = {
            "provider": str(settings.rag_embedding_provider or ""),
            "normalized_provider": provider,
            "model": model_name,
            "custom_embedding_requested": provider not in {"", "default", "chroma", "chroma_default", "none"},
            "custom_embedding_enabled": False,
            "factory_name": None,
        ...
        spec = cls._embedding_provider_spec(provider)
        if spec is None:
            return {**default_status, "fallback_reason": f"unsupported_provider:{provider}"}

        factory = cls._embedding_factory(embedding_functions_module, spec)
        dependency_available = cls._embedding_dependency_available(
            spec["dependency"],
            dependency_checker,
        )
        api_key_configured = True
        if spec["api_key_required"]:
            api_key_configured = bool(cls._embedding_api_key(settings, provider))
    ...
    def _build_embedding_function(
        ...
        status = cls.embedding_provider_status(
            settings=settings,
        ...
        model_name = str(settings.rag_embedding_model or "").strip()
        spec = cls._embedding_provider_spec(provider) or {}
        factory = cls._embedding_factory(embedding_functions_module, spec)
        if factory is None:
            return None
        try:
            ...
            if provider == "openai":
                return factory(api_key=cls._embedding_api_key(settings, provider), model_name=model_name)
            if provider in {"google_genai", "google_sdk", "gemini_genai"}:
                return factory(
                    api_key=cls._embedding_api_key(settings, provider),
    ...
    def _collection_name_for_settings(
        ...
        provider = str(getattr(settings, "rag_embedding_provider", "") or "chroma_default").lower().replace("-", "_")
        model = str(getattr(settings, "rag_embedding_model", "") or "default").lower()
        schema_version = cls._index_schema_version(settings)
        if (
            not embedding_function_available
            or provider in {"", "default", "chroma", "chroma_default", "none"}
        ):
            provider = "chroma_default"
            model = "default"
        digest = sha1(f"{provider}|{model}|{schema_version}".encode("utf-8")).hexdigest()[:10]
        base = cls._collection_name_part(collection_name, 24)
        provider_label = cls._collection_name_part(provider or "chroma_default", 18)
        schema_label = cls._collection_name_part(schema_version, 16)
