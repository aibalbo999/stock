...
from app.rag.hybrid_retrieval_ranker import (
    HybridRetrievalRanker,
    bm25_scores,
    document_key,
    document_matches_target_tickers,
    elapsed_ms,
    exact_phrase_boost,
    filter_by_target_tickers,
    is_confusing_entity_prefix_token,
    keyword_document_text,
    source_quality_multiplier_for_document,
    source_quality_tier_for_document,
    target_aliases_by_ticker,
    tokenize,
    trace_row,
)
from app.rag.rag_document_codec import (
    EMBEDDING_BODY_MARKER as _EMBEDDING_BODY_MARKER,
    document_from_metadata,
    embedding_document_text,
    metadata_for_document,
    metadata_list,
    stored_document_body,
)
...
EMBEDDING_BODY_MARKER = _EMBEDDING_BODY_MARKER
...
class VectorStore:
    ...
    def _document_from_metadata(document_id: str, text: str, metadata: dict) -> NewsDocument:
        return document_from_metadata(document_id, text, metadata)

    @staticmethod
    def _metadata_for_document(document: NewsDocument) -> dict:
        return metadata_for_document(document)
    ...
    def _embedding_document_text(document: NewsDocument, metadata: dict | None = None) -> str:
        return embedding_document_text(document, metadata)
    ...
    def _stored_document_body(text: str) -> str:
        return stored_document_body(text)
    ...
    def _hybrid_rank(
        ...
        *,
        ...
        results, trace = HybridRetrievalRanker(self.settings, self.reranker).rank(
            query,
            vector_hits,
            keyword_corpus,
            n_results,
    ...
    def _trace_row(
        ...
        *,
        ...
        return trace_row(
            ...
            source_quality_multiplier=source_quality_multiplier,
    ...
    def _source_quality_multiplier(document: NewsDocument) -> float:
        return source_quality_multiplier_for_document(document)

    @staticmethod
    def _source_quality_tier(document: NewsDocument) -> str:
        return source_quality_tier_for_document(document)
    ...
    def _filter_by_target_tickers(
        ...
        return filter_by_target_tickers(documents, target_tickers, target_aliases)
    ...
    def _document_matches_target_tickers(
        ...
        return document_matches_target_tickers(document, target_tickers, target_aliases)
