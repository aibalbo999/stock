...
def format_external_deployment_env_template(report: dict[str, Any]) -> str:
    rows = report.get("rows") if isinstance(report.get("rows"), list) else []
    lines = [
        "# External deployment env template generated by scripts/external_deployment_env_gaps.py",
        "# Review before use. Do not commit real secrets.",
    ]
    if not rows:
        lines.append("# No external deployment env gaps detected.")
        return "\n".join(lines)
    for entry in _external_env_template_entries(rows):
        lines.append("")
        lines.append(
...
def _external_env_template_entries(rows: list[dict]) -> list[dict]:
    grouped: dict[str, list[dict]] = {}
    for row in rows:
        if not isinstance(row, dict):
            continue
        env_key = str(row.get("設定鍵") or "").strip()
        if not env_key:
            continue
        grouped.setdefault(env_key, []).append(row)
    entries = []
    for env_key, env_rows in grouped.items():
        row = sorted(env_rows, key=_external_env_key_row_sort_key)[0]
        value = _external_env_template_value(
            env_key=env_key,
            recommended_value=str(row.get("建議值") or ""),
...
def _external_env_template_value(
    *,
    ...
    value = str(recommended_value or "").strip() or "-"
    if value == "-":
        value = str(EXTERNAL_ENV_KEY_HINTS.get(env_key, {}).get("default") or "<set-manually>")
