...
def format_external_deployment_env_check_report(report: dict[str, Any]) -> str:
    lines = [
        (
            ...
            f"(target={report['target']}; checked={report['checked_count']}; "
            f"missing={report['missing_count']}; different={report['different_count']})"
        )
    ]
    if report.get("env_file"):
        exists = "exists" if report.get("env_file_exists") else "missing"
        lines.append(f"Env file: {report['env_file']} ({exists})")
    if not report.get("rows"):
        lines.append("No external deployment env keys to check.")
        return "\n".join(lines)
    for row in report["rows"]:
        lines.append(
            f"- [{row['status']}] {row['env_key']} :: expected={row['expected_value']} "
