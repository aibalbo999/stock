...
class SupplyChainGraph:
    ...
    def render_reasoning_context(self, paths_by_ticker: dict[str, list[dict]], topic: str = "") -> str:
        lines = [
            "### GraphRAG 路徑推理",
        ...
        while queue and len(paths) < max_paths:
            ...
            for neighbor, edge in adjacency.get(current, []):
                if neighbor in visited:
                    continue
                queue.append((neighbor, [*path_edges, (current, neighbor, edge)], visited | {neighbor}))
        return paths

    def _path_payload(self, source: str, path_edges: list[tuple[str, str, SupplyChainEdge]]) -> dict:
        node_by_ticker = self._node_by_ticker()
    ...
    def _build_edges(
        ...
        segment_categories = {
            segment.id: SupplyChainGraph.classify_segment(segment)
            for segment in segments
        }
        edges: list[SupplyChainEdge] = []
        seen = set()
        ...
        return sorted(edges, key=lambda edge: (edge.source_ticker, edge.target_ticker, edge.relation))

    @staticmethod
    def _peer_edges(nodes: list[SupplyChainNode], seen: set[tuple[str, str, str]]) -> list[SupplyChainEdge]:
        edges = []
...
def _direction_evidence_terms(direction: str) -> tuple[str, ...]:
