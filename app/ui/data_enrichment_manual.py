...
def _render_manual_news_form() -> None:
    ...
    render_data_ingest_submission_summary(
        manual_news_preflight_summary(
            ...
            ready=manual_news_ready,
            confirmed=manual_news_confirmed,
        ),
        streamlit_module=st,
    )
    if st.button(
...
def _render_company_filing_form(whitelist: Any, allowed_tickers: list[str]) -> None:
    ...
    filing_type = st.selectbox(
        ...
        format_func=company_filing_type_label,
    )
    ...
    if filing_text_ready:
        render_data_ingest_submission_summary(
            company_filing_text_preflight_summary(
                ...
                title=filing_title,
                ready=filing_text_ready,
                confirmed=filing_text_confirmed,
            ),
            streamlit_module=st,
        )
    ...
    if filing_url_ready:
        render_data_ingest_submission_summary(
            company_filing_url_preflight_summary(
                ...
                document_type=filing_type,
                url=filing_url,
                ready=filing_url_ready,
                confirmed=filing_url_confirmed,
            ),
            streamlit_module=st,
        )
...
def company_filing_type_label(document_type: str) -> str:
    return COMPANY_FILING_TYPE_LABELS.get(str(document_type or "").strip(), str(document_type))
